...
function get_codepoint(node::EzXML.Node)
    cps = try
...
UAX#42
# Fields
- `type`: `char`, `reserved`, `noncharacter`, `surrogate`
- `cp`: single code point or range with first, last
- `na`: standard name
- `na1`: 1.0 name
- `name_alias`: zero or more
    - `alias`: String
    - `type`: "abbreviation", "alternate", ... etc
- `blk`: block
- `gc`: general category (enum)

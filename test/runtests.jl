...
@testset "UCDxml.jl" begin
    ...
    @testset "Type and func test" begin
        ...
        @test UCDxml.RangeCodePoint(UInt32(0x0), UInt32(0x100)) ==
              UCDxml.RangeCodePoint(UInt32(0x0), UInt32(0x100))
        @test UCDxml.char == UCDxml.char
        @test UCDxml.NameAlias("NUL", "abbreviation") ==
              UCDxml.NameAlias("NUL", "abbreviation")
        @test [
            UCDxml.NameAlias("NUL", "abbreviation"),
            UCDxml.NameAlias("NULL", "control"),
        ] == [UCDxml.NameAlias("NUL", "abbreviation"), UCDxml.NameAlias("NULL", "control")]
    ...
    @testset "Parse result check" begin
        ...
        if full_test
            @test ucd_repertoire[0x30E8] == UCDxml.UCDRepertoireNode(UCDxml.char,
                                           ...
                                           UInt8(0),
                                           UCDxml.BidirectionalProperties(UCDxml.BidirectionalClasses.L,
                                                                          false, nothing,
                                                                          false,
                                                                          UCDxml.BidiPairedBracketTypes.n,
                                                                          UCDxml.SingleCodePoint(UInt32(0x3402))),
                                           UCDxml.DecompositionProperties(UCDxml.DecompositionTypes.none,
                                                                          UCDxml.SingleCodePoint(UInt32(0x3402)),
                                                                          false, false,
                                                                          UCDxml.QuickCheckProperties.Yes,
                                                                          UCDxml.QuickCheckProperties.Yes,
                                                                          UCDxml.QuickCheckProperties.Yes,
                                                                          UCDxml.QuickCheckProperties.Yes,
                                                                          false, false,
                                                                          false, false,
                                                                          UCDxml.SingleCodePoint(UInt32(0x3402))),
                                           UCDxml.NumericProperties(UCDxml.NumericTypes.None,
                                                                    NaN),
                                           UCDxml.JoiningProperties(UCDxml.JoiningTypes.U,
                                                                    "No_Joining_Group",
                                                                    false),
    ...
    @testset "Write result test" begin
        ...
        @testset "Modules" begin
            ...
            @test UCDxml.out_codepoint(char_0x0000.cp) ==
                  "UCDxml.SingleCodePoint(UInt32(0x0000))"
            @test (x -> (UCDxml.out_codepoint(x) |> Meta.parse |> eval) == x)(char_0x0000.cp)
            @test UCDxml.out_name_alias_vec(char_0x0000.name_alias) ==
                  "[UCDxml.NameAlias(\"NUL\", \"abbreviation\"), UCDxml.NameAlias(\"NULL\", \"control\")]"
            ...
            @test (x -> (("UCDxml.GeneralCategories." * UCDxml.out_general_category(x)) |>
                         Meta.parse |> eval) == x)(char_0x0000.gc)
        end
        ...
        @testset "Write and parse all UCDRepertoireNode" begin for ucd_node in ucd_repertoire
    ...
    @testset "Exported result check" begin
        @testset "Check all hardcoded data is correct" begin for (raw_ucd, hardcoded_ucd) in zip(ucd_repertoire,
                                                                                                 UCDxml.ucd_list)
